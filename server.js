...
async function initDB() {
    try {
        ...
        await client.query(`CREATE TABLE IF NOT EXISTS products (id SERIAL PRIMARY KEY, name TEXT, price TEXT, stock INTEGER, category TEXT, description TEXT, type TEXT DEFAULT 'virtual', image_url TEXT, is_pinned BOOLEAN DEFAULT FALSE, created_at TIMESTAMP DEFAULT NOW())`);
        
        await client.query(`CREATE TABLE IF NOT EXISTS orders (id SERIAL PRIMARY KEY, order_id TEXT UNIQUE, product_name TEXT, contact TEXT, payment_method TEXT, status TEXT DEFAULT '待支付', user_id INTEGER, usdt_amount NUMERIC, cny_amount NUMERIC, snapshot_rate NUMERIC, shipping_info TEXT, expires_at TIMESTAMP, tracking_number TEXT, fee_amount NUMERIC DEFAULT 0, qrcode_url TEXT, created_at TIMESTAMP DEFAULT NOW())`);
...
// --- USDT Check (Simplified for demo) ---
...
app.post('/api/user/login', async (req, res) => {
    ...
    try {
        ...
        if (user.rows.length === 0) {
            return res.json({ success: false, msg: "账号不存在，请先注册" });
        } else {
...
app.post('/api/user/register', async (req, res) => {
    ...
    try {
        let user = await pool.query("SELECT * FROM users WHERE contact = $1", [contact]);
        if (user.rows.length > 0) {
            return res.json({ success: false, msg: "该账号已存在，请直接登录" });
        } else {
...
// Upload QR Code for Order (User/Admin)
